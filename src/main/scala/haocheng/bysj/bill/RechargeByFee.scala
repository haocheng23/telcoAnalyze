...
object RechargeByFee {
  def main(args: Array[String]): Unit = {
    ...
    /*创建临时视图*/
    DF.createTempView("res_recharge_fee")

    /*执行sql语句*/
    val frame: DataFrame = session.sql("SELECT rechargefee,COUNT(*) total FROM res_recharge_fee GROUP BY rechargefee ")
    ...
    /*将结果写入mysql*/
    frame.write.mode("overwrite")jdbc(url,"res_recharge_fee",connprop)
  }
